...
import {
  ...
  Grid, IconButton, MenuItem, Skeleton, Stack, Tab, Tabs, TextField, Tooltip, Typography,
} from '@mui/material';
    ...
    <Box>
      <PageHeader
        ...
        action={
          <Stack direction="row" spacing={1}>
            ...
            {(can(PERMISSIONS.REQUEST_STATUS_CHANGE) || can(PERMISSIONS.REQUEST_ASSIGN)) && (
              <Button
                variant="contained"
                size="small"
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 340px' }, gap: 2.5, alignItems: 'start' }}>
        ...
        <Box sx={{ minWidth: 0 }}>
          ...
          {tab === 0 && (
            <Stack spacing={2}>
              {/* Quick Add Remark box right in the feed */}
              ...
              {/* Combined Timeline & Remarks Feed */}
              <Card>
                <CardHeader
          ...
          {tab === 1 && (
            <Stack spacing={2}>
              {can(PERMISSIONS.DOCUMENT_UPLOAD) && (
                <Card>
                  <CardHeader
                    title="Upload or Scan Citizen Papers"
                    titleTypographyProps={{ variant: 'subtitle1', fontWeight: 700 }}
                    avatar={<Icon name="CloudUpload" sx={{ color: 'primary.main' }} />}
                  />
                  <Divider />
                  <CardContent>
                    <DocumentUploader owner={{ kind: 'request', id }} onUploaded={refresh} />
                  </CardContent>
                </Card>
              )}
              <Card>
